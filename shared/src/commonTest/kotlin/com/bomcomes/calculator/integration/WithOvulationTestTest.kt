...
/**
 * 배란 테스트 기록 검증 테스트 (데이터 기반)
 *
 * 문서 참조: test-cases/docs/05-with-ovulation-test.md
 *
 * 배란 테스트 양성 기록이 있을 때의 계산 로직 검증
 */
class WithOvulationTestTest {
    ...
    companion object {
        ...
        // 배란 테스트 기록 (모두 양성)
        val OVULATION_TEST_1 = LocalDate(2025, 1, 15)
        val OVULATION_TEST_2 = LocalDate(2025, 2, 11)
        val OVULATION_TEST_3 = LocalDate(2025, 2, 12)
        val OVULATION_TEST_4 = LocalDate(2025, 3, 18)
    ...
    private fun setupCommonData(repository: InMemoryPeriodRepository) {
        ...
        repository.addOvulationTest(OvulationTest(
            date = DateUtils.toJulianDay(OVULATION_TEST_1),
            result = TestResult.POSITIVE
        ))
        repository.addOvulationTest(OvulationTest(
            date = DateUtils.toJulianDay(OVULATION_TEST_2),
            result = TestResult.POSITIVE
        ))
        repository.addOvulationTest(OvulationTest(
            date = DateUtils.toJulianDay(OVULATION_TEST_3),
            result = TestResult.POSITIVE
        ))
        repository.addOvulationTest(OvulationTest(
            date = DateUtils.toJulianDay(OVULATION_TEST_4),
            result = TestResult.POSITIVE
        ))
